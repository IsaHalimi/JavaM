...
public class Collision {
	...
	// Prüft, ob der gegebene Block in die gegebene Richtung stoßen würde.
	public static boolean collideWithBlock(Block b, int direction) {
		// direction: -1 = links, 0 = runter, 1 = rechts

		// direction: -1 = links, 0 = runter, 1 = rechts
		// Prüft je nach Richtung verschiedene Kollisionsszenarien
		switch (direction) {
		// Falls die Richtung links ist
		case -1:
			// Prüft die Kollision des Blocks mit dem linken Rand.
			if (b.getY() > 0) {
				if (b.getX() > 0) {
			...
			break;
		// Falls die Richtung unten ist
		case 0:
			// Prüft die Kollision des Blocks mit dem Boden.
			...
			break;
			// Falls die Richtung rechts ist
		case 1:
			// Prüft die Kollision des Blocks mit dem rechten Rand.
			if (b.getY() > 0) {
				if (b.getX() < 10) {
					for (int i = 0; i < b.getBounds()[b.getRotation()].length; i++) {
						for (int j = 0; j < b.getBounds()[b.getRotation()][i].length; j++) {
	...
	// Prüft, ob der Block bei einer Drehung mit anderen Blöcken kollidiert
	...
	// Prüft, ob der Block in die angegebene Richtung mit der Wand kollidiert
	...
	// Füllt den Block auf der Karte aus.
	...
	// Prüft, ob eine vollständige Zeile vorhanden ist und löscht diese
	...
	// Löscht die angegebene Zeile und bewegt alle Zeilen darüber eine Zeile nach unten
	...
	// Prüft, ob der Spieler verloren hat (wenn ein Block die oberste Zeile erreicht)
